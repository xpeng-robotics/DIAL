...
"""
DIAL Inference Service

This script provides both ZMQ and HTTP server/client implementations for deploying DIAL models.
...
Dependencies for `http_server` mode:
    => Server (runs DIAL model on GPU): `pip install uvicorn fastapi json-numpy`
...
def main(args: ArgsConfig):
    if args.server:
        # Create a policy
        # The `DialPolicy` class is being used to create a policy object that encapsulates
        ...
        policy = DialPolicy(
            ...
            async_vlm_inference=args.async_vlm_inference,
            vlm_update_period=args.vlm_update_period,
        )
