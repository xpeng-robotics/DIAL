...
class SimulationInferenceClient(BaseInferenceClient, BasePolicy):
    ...
    def setup_environment(self, config: SimulationConfig) -> gym.vector.VectorEnv:
        ...
        return _create_single_env(config=config, idx=0)
    ...
    def run_simulation(self, config: SimulationConfig) -> Tuple[str, List[bool]]:
        ...
        episode_lengths = []
        current_rewards = [0] * config.n_episodes
        current_lengths = [0] * config.n_episodes
        completed_episodes = 0
        current_successes = [False] * config.n_episodes
        episode_successes = []
        # Initial environment reset
        obs, _ = self.env.reset()
        # Main simulation loop
        for ep_id in range(config.n_episodes):
            while True:
                # Process observations and get actions from the server
                obs = {k: v[None, :] if not type(v) == str else (v,) for k, v in obs.items()}
                ...
                current_successes[ep_id] |= bool(env_infos["success"][0])
                current_rewards[ep_id] += rewards
                current_lengths[ep_id] += 1
                obs = next_obs

                # If episode ended, store results
                if terminations or truncations:
                    episode_lengths.append(current_lengths[ep_id])
                    episode_successes.append(current_successes[ep_id])
