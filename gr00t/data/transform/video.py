...
class VideoOffsetCrop(VideoTransform):
    ...
    """
    top: int = Field(..., description="Pixel position of the top edge of the crop area.")
    left: int = Field(..., description="Pixel position of the left edge of the crop area.")
    height: int = Field(..., description="Height of the crop area.")
    width: int = Field(..., description="Width of the crop area.")

    def get_transform(self, mode: Literal["train", "eval"] = "train") -> Callable:
        """
        Get the crop transform function.
        
        Args:
            mode (Literal["train", "eval"]): Mode (does not affect behavior in this class).

        Returns:
            Callable: Transform function to apply to video tensors.
        """
        if self.backend == "torchvision":
            # torchvision.transforms.functional.crop can directly handle (..., H, W) tensors,
            # so it can be applied directly to (T, C, H, W) video tensors, cropping each frame identically.
        ...
        # You can add implementations for other backends (e.g. albumentations) as needed
        elif self.backend == "albumentations":
            # Albumentations Crop requires a different implementation, omitted for now
            raise NotImplementedError("VideoOffsetCrop not implemented for albumentations backend yet.")
        else:
            raise ValueError(f"Backend {self.backend} not supported")

    def check_input(self, data: dict[str, Any]):
        # You can keep or implement input validation as needed
        pass
